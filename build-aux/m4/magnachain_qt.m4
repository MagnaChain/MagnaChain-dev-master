...
AC_DEFUN([MAGNACHAIN_QT_FAIL],[
  if test "x$magnachain_qt_want_version" = "xauto" && test x$magnachain_qt_force != xyes; then
    ...
    magnachain_enable_qt=no
    magnachain_enable_qt_test=no
  else
    AC_MSG_ERROR([$1])
  fi
])
...
AC_DEFUN([MAGNACHAIN_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [build magnachain-qt GUI (default=auto, qt5 tried first)])],
    [
     magnachain_qt_want_version=$withval
     if test x$magnachain_qt_want_version = xyes; then
       magnachain_qt_force=yes
       magnachain_qt_want_version=auto
     fi
    ],
    [magnachain_qt_want_version=auto])
...
AC_DEFUN([MAGNACHAIN_QT_CONFIGURE],[
  ...
  if test x$magnachain_qt_got_major_vers = x5; then
    ...
    if test x$magnachain_cv_static_qt = xyes; then
      ...
      AC_CACHE_CHECK(for Qt < 5.4, magnachain_cv_need_acc_widget,[AC_COMPILE_IFELSE([AC_LANG_PROGRAM(
        ...
        [magnachain_cv_need_acc_widget=yes],
  ...
  MAGNACHAIN_QT_CHECK([
    ...
    magnachain_enable_qt_dbus=no
    if test x$use_dbus != xno && test x$have_qt_dbus = xyes; then
      magnachain_enable_qt_dbus=yes
    fi
  ...
  AC_MSG_RESULT([$magnachain_enable_qt (Qt${magnachain_qt_got_major_vers})])
  ...
  AC_SUBST(QT_SELECT, qt${magnachain_qt_got_major_vers})
  AC_SUBST(MOC_DEFS)
])
...
AC_DEFUN([_MAGNACHAIN_QT_CHECK_QT5],[
  ...
  AC_COMPILE_IFELSE([AC_LANG_PROGRAM(
    ...
    [magnachain_cv_qt5=yes],
    [magnachain_cv_qt5=no])
])])
...
AC_DEFUN([_MAGNACHAIN_QT_IS_STATIC],[
  ...
  AC_COMPILE_IFELSE([AC_LANG_PROGRAM(
    ...
    [magnachain_cv_static_qt=yes],
    [magnachain_cv_static_qt=no])
  ])
  if test xmagnachain_cv_static_qt = xyes; then
...
AC_DEFUN([_MAGNACHAIN_QT_FIND_STATIC_PLUGINS],[
  if test x$magnachain_qt_got_major_vers = x5; then
     ...
     m4_ifdef([PKG_CHECK_MODULES],[
       ...
       if test x$TARGET_OS = xwindows; then
         AC_CACHE_CHECK(for Qt >= 5.6, magnachain_cv_need_platformsupport,[AC_COMPILE_IFELSE([AC_LANG_PROGRAM(
             [[#include <QtCore>]],[[
             #if QT_VERSION < 0x050600
             choke;
             #endif
             ]])],
           [magnachain_cv_need_platformsupport=yes],
...
dnl Inputs: magnachain_qt_want_version (from --with-gui=). The version to check
dnl         first.
dnl Inputs: $1: If magnachain_qt_want_version is "auto", check for this version
dnl         first.
...
AC_DEFUN([_MAGNACHAIN_QT_FIND_LIBS_WITH_PKGCONFIG],[
  ...
  if test x$auto_priority_version = x; then
    ...
    if test x$magnachain_qt_want_version = xqt5 ||  ( test x$magnachain_qt_want_version = xauto && test x$auto_priority_version = xqt5 ); then
      QT_LIB_PREFIX=Qt5
      magnachain_qt_got_major_vers=5
    else
      QT_LIB_PREFIX=Qt
      magnachain_qt_got_major_vers=4
    fi
    ...
    MAGNACHAIN_QT_CHECK([
      ...
      if test x$have_qt = xno && test x$magnachain_qt_want_version = xauto; then
        if test x$auto_priority_version = xqt5; then
          PKG_CHECK_MODULES([QT], [$qt4_modules], [QT_INCLUDES="$QT_CFLAGS"; have_qt=yes; QT_LIB_PREFIX=Qt; magnachain_qt_got_major_vers=4], [have_qt=no])
        else
          PKG_CHECK_MODULES([QT], [$qt5_modules], [QT_INCLUDES="$QT_CFLAGS"; have_qt=yes; QT_LIB_PREFIX=Qt5; magnachain_qt_got_major_vers=5], [have_qt=no])
        fi
      fi
