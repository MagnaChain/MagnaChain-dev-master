...
License:	MIT
URL:		https://magnachain.org/
Source0:	https://magnachain.org/bin/magnachain-core-%{version}/magnachain-%{version}.tar.gz
...
%package devel
Summary:	Development files for magnachain
...
%package server
Summary:	The magnachain daemon
Group:		System Environment/Daemons
Requires:	magnachain-utils = %{version}-%{release}
...
%description server
This package provides a stand-alone magnachain-core daemon. For most users, this
package is only needed if they need a full-node without the graphical client.

Some third party wallet software will want this package to provide the actual
magnachain-core node they use to connect to the network.

If you use the graphical magnachain-core client then you almost certainly do not
need this package.
...
cp -p %{SOURCE10} ./magnachain.conf.example
tar -zxf %{SOURCE1}
...
for selinuxvariant in %{selinux_variants}; do
	...
	mv magnachain.pp magnachain.pp.${selinuxvariant}
...
cat <<EOF > %{buildroot}%{_sysconfdir}/sysconfig/magnachain
# Provide options to the magnachain daemon here, for example
...
EOF
touch -a -m -t 201504280000 %{buildroot}%{_unitdir}/magnachain.service
#end systemd stuff

mkdir %{buildroot}%{_sysconfdir}/magnachain
mkdir -p %{buildroot}%{_localstatedir}/lib/magnachain

#SELinux
for selinuxvariant in %{selinux_variants}; do
	install -d %{buildroot}%{_datadir}/selinux/${selinuxvariant}
	install -p -m 644 SELinux/magnachain.pp.${selinuxvariant} %{buildroot}%{_datadir}/selinux/${selinuxvariant}/magnachain.pp
done

%if %{_buildqt}
# qt icons
install -D -p share/pixmaps/magnachain.ico %{buildroot}%{_datadir}/pixmaps/magnachain.ico
...
touch -a -m -t 201511100546 %{buildroot}%{_datadir}/applications/magnachain-core.desktop
%{_bindir}/desktop-file-validate %{buildroot}%{_datadir}/applications/magnachain-core.desktop

# KDE protocol - change the touch timestamp if modifying
mkdir -p %{buildroot}%{_datadir}/kde4/services
cat <<EOF > %{buildroot}%{_datadir}/kde4/services/magnachain-core.protocol
[Protocol]
exec=magnachain-qt '%u'
protocol=magnachain
input=none
...
touch -a -m -t 201511100546 %{buildroot}%{_datadir}/kde4/services/magnachain-core.protocol
%endif
...
%check
make check
srcdir=src test/magnachain-util-test.py
...
%post server
%systemd_post magnachain.service
# SELinux
...
%preun server
%systemd_preun magnachain.service

%postun server
%systemd_postun magnachain.service
# SELinux
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	...
	for selinuxvariant in %{selinux_variants}; do
		%{_sbindir}/semodule -s ${selinuxvariant} -r magnachain &> /dev/null || :
	done
	%{_sbindir}/fixfiles -R magnachain-server restore &> /dev/null || :
	[ -d %{_localstatedir}/lib/magnachain ] && \
		%{_sbindir}/restorecon -R %{_localstatedir}/lib/magnachain &> /dev/null || :
	fi
fi
...
%doc COPYING magnachain.conf.example doc/README.md doc/bips.md doc/files.md doc/multiwallet-qt.md doc/reduce-traffic.md doc/release-notes.md doc/tor.md
%attr(0755,root,root) %{_bindir}/magnachain-qt
%attr(0644,root,root) %{_datadir}/applications/magnachain-core.desktop
%attr(0644,root,root) %{_datadir}/kde4/services/magnachain-core.protocol
%attr(0644,root,root) %{_datadir}/pixmaps/*.ico
%attr(0644,root,root) %{_datadir}/pixmaps/*.bmp
%attr(0644,root,root) %{_datadir}/pixmaps/*.svg
%attr(0644,root,root) %{_datadir}/pixmaps/*.png
%attr(0644,root,root) %{_datadir}/pixmaps/*.xpm
%attr(0644,root,root) %{_mandir}/man1/magnachain-qt.1*
%endif
...
- Rename Qt package from magnachain to magnachain-core
...
# based upon the 0.11.2 package spec file from https://www.ringingliberty.com/magnachain/
