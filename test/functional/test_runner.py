...
`test/functional/test_framework/test_framework.py:MagnaChainTestFramework.main`.
...
def get_python():
    if sys.platform == 'win32':
        return 'python.exe'
    return os.popen('which python3').read().strip()
...
def main():
    ...
    # configfile = os.path.abspath(os.path.dirname(__file__)) + os.path.join('..','config.ini')#/../config.ini"
    cur_dir = os.path.abspath(os.path.dirname(__file__))
    configfile = os.path.join(os.path.dirname(cur_dir),'config.ini')
    ...
    tmpdir = "%s/magnachain_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    tmpdir = "%s" % (os.path.join(args.tmpdirprefix,'magnachain_test_runner_' + datetime.datetime.now().strftime("%Y%m%d_%H%M%S")))
    os.makedirs(tmpdir)
    ...
    enable_magnachaind = config["components"].getboolean("ENABLE_MAGNACHAIND")


    if config["environment"]["EXEEXT"] == ".exe" and not args.force:
        # https://github.com/magnachain/magnachain/commit/d52802551752140cf41f0d9a225a43e84404d3e9
        # https://github.com/magnachain/magnachain/pull/5677#issuecomment-136646964
        print("Tests currently disabled on Windows by default. Use --force option to enable")
        sys.exit(0)

    if not (enable_wallet and enable_utils and enable_magnachaind):
        print("No functional tests to run. Wallet, utils, and magnachaind must all be enabled")
    ...
    if args.help:
        ...
        subprocess.check_call(
            [os.path.join((config["environment"]["SRCDIR"],"test",'functional',test_list[0].split()[0]))] + ['-h'])
            # [(config["environment"]["SRCDIR"] + '/test/functional/' + test_list[0].split()[0])] + ['-h'])
        sys.exit(0)
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[]):
    ...
    try:
        if subprocess.check_output(["pidof", "magnachaind"]) is not None:
            print("%sWARNING!%s There is already a magnachaind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "MAGNACHAIND" not in os.environ:
        # os.environ["MAGNACHAIND"] = build_dir + '/src/magnachaind' + exeext
        os.environ["MAGNACHAIND"] = os.path.join(build_dir,'src','magnachaind' + exeext)
        # os.environ["MAGNACHAINCLI"] = build_dir + '/src/magnachain-cli' + exeext
        os.environ["MAGNACHAINCLI"] = os.path.join(build_dir, 'src', '/magnachain-cli' + exeext)

    # tests_dir = src_dir + '/test/functional/'
    tests_dir = os.path.join(src_dir,'test','functional')

    # flags = ["--srcdir={}/src".format(build_dir)] + args
    flags = ["--srcdir={}".format(os.path.join(build_dir,'src'))] + args
...
class TestHandler:
    ...
    def __init__(self, num_tests_parallel, tests_dir, tmpdir, test_list=None, flags=None):
        ...
        self.flags = flags
        self.num_running = 0
        # In case there is a graveyard of zombie magnachainds, we can apply a
    ...
    def get_next(self):
        while self.num_running < self.num_jobs and self.test_list:
            ...
            tmpdir = ["--tmpdir={}".format(os.path.join(self.tmpdir,re.sub(".py$", "", test_argv[0]), str(portseed)))]
            # print(self.tests_dir , test_argv[0] , test_argv[1:] , self.flags , portseed_arg , tmpdir)
            # print([os.path.join(self.tests_dir , test_argv[0])] + test_argv[1:] + self.flags + portseed_arg + tmpdir)
            self.jobs.append((t,
                              time.time(),
                              subprocess.Popen([get_python(),os.path.join(self.tests_dir , test_argv[0])] + test_argv[1:] + self.flags + portseed_arg + tmpdir,
        ...
        while True:
            ...
            for j in self.jobs:
                ...
                if proc.poll() is not None:
                    ...
                    if sys.platform == 'win32':
                        [stdout, stderr] = [l.read().decode('gbk') for l in (log_out, log_err)]
                    else:
...
class RPCCoverage(object):
    ...
    commands per `magnachain-cli help` (`rpc_interface.txt`).
