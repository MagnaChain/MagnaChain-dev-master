...
"""Class for magnachaind node under test"""
...
class TestNode():
    ...
    def __init__(self, i, dirname, extra_args, rpchost, timewait, binary, stderr, mocktime, coverage_dir):
        ...
        self.args = [self.binary, "-datadir=" + self.datadir, "-server", "-keypool=1", "-discover=0", "-rest", "-logtimemicros", "-debug", "-debugexclude=libevent", "-debugexclude=leveldb", "-mocktime=" + str(mocktime), "-uacomment=testnode%d" % i, "-powtargetspacing=1"]
    ...
    def wait_for_rpc_connection(self):
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            ...
            except ValueError as e:  # cookie file not found and no rpcuser or rpcassword. magnachaind still starting
                if "No RPC credentials" not in str(e):
                    raise
            time.sleep(1.0 / poll_per_s)
        raise AssertionError("Unable to connect to magnachaind")
    ...
    def wait_until_stopped(self, timeout=MAGNACHAIND_PROC_WAIT_TIMEOUT):
        wait_until(self.is_node_stopped, timeout=timeout)

    def node_encrypt_wallet(self, passphrase):
        """"Encrypts the wallet.

        This causes magnachaind to shutdown, so this method takes
...
class TestNodeCLI():
    ...
    def __call__(self, *args, input=None):
        # TestNodeCLI is callable with magnachain-cli command-line args
    ...
    def send_cli(self, command, *args, **kwargs):
        """Run magnachain-cli command. Deserializes returned string as python object."""
