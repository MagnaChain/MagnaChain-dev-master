...
class ContractCallTest(MagnaChainTestFramework):
    ...
    def run_test(self):
        ...
        print("balance:++++",node.getbalance())
        list(map(lambda to: caller_tmp("dustChangeTest", to,amount = Decimal("0")), senders)) # 向每个地址发送(cell / 11) * 10(最小单位)，cell = 100000000。这里应该有11个微交易的找零
        print("balance:----", node.getbalance())
        node.generate(nblocks=2)
        tmp_sender = node.getnewaddress()
        caller_tmp("sendCoinTest",tmp_sender,1,amount = Decimal("0")) # 组合所有微交易的找零，应该足够1个MGC的
        node.generate(nblocks=2)
        print(node.getbalanceof(tmp_sender))
