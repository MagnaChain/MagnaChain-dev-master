...
function init()
	...
	PersistentData.kittyIndexToOwner = {} --player address to his kitty id
...
local function _createKitty( _matronId, _sireId, _generation, _genes, _owner )	
	...
	table.insert(PersistentData.kitties, _kitty)
...
local function _isReadyToBreed( kitty )
	return kitty.siringWithId == 0 and kitty.cooldownEndBlock <= msg.blocknumber
end
...
local function _triggerCooldown( kitty )
	kitty.cooldownEndBlock = cooldowns[kitty.cooldownIndex]/PersistentData.secondsPerBlock + msg.blocknumber
	
	if kitty.cooldownIndex < 13 then
		kitty.cooldownIndex = kitty.cooldownIndex + 1
	end
end
...
local function _isReadyToGiveBirth( kittyMatron )
	return kittyMatron.siringWithId ~= 0 and kittyMatron.cooldownEndBlock <= msg.blocknumber
end
...
function isReadyToBreed( params )
	...
	assert(_kittyId > 0)
	local kitty = PersistentData.kitties[_kittyId]
	local b = _isReadyToBreed( kitty )
	return {b}
end
...
--
local function _isValidMatingPair( _kittyMatron, _matronId, _kittySire, _sireId )
	...
	if _kittyMatron.matronId == _sireId or _kittyMatron.sireId == _sireId then
		return false
	end
	if _kittySire.matronId == _matronId or _kittySire.sireId == _matronId then
		return false
	end
	
	--We can short circuit the sibling check (below) if either cat is
	--gen zero (has a matron ID of zero).
	if _kittySire.matronId == 0 or _kittyMatron.matronId == 0 then
		return true
	end
	
	--Kitties can't breed with full or half siblings.
	if _kittySire.matronId == _kittyMatron.matronId or _kittySire.matronId == _kittyMatron.sireId then
		return false
	end
	if _kittySire.sireId == _kittyMatron.matronId or _kittySire.sireId == _kittyMatron.sireId then
		return false
	end
	return true
end
...
function giveBirth( params )
	...
	local kittyId = _createKitty(_matronId, matron.siringWithId, parentGen + 1, childGenes, owner)
...
function createPromoKitty(params)
	...
	local kittyId = _createKitty(0, 0, 0, _genes, kittyOwner)
	return kittyId
end
