...
extern int COINBASE_MATURITY; // @note regtest's maturity is 1, modify in init.cpp
...
extern const int BRANCH_CHAIN_CREATE_COIN_MATURITY; // 半年才能赎回, 527040块 * 30s/块 = 183天 . 设定比较长的时间主要防止恶意创建很多支链。
extern int BRANCH_CHAIN_MATURITY;// 至少需要 2000 块 * 30s/块 = 1000 分钟 = 16.67 hours
extern const MCAmount MIN_MINE_BRANCH_MORTGAGE; // 抵押挖矿最小值
extern int REDEEM_SAFE_HEIGHT; // 10800 * 8s = 1 day (branch chain block time) 挖矿币安全高度（继续挖矿和赎回需要满足的高度）、举报高度
extern int REPORT_OUTOF_HEIGHT; // 2880 * 30s = 1 day
extern int REPORT_LOCK_COIN_HEIGHT; // 30 * 30s = 15 mins
