...
int32_t COINBASE_MATURITY = 500; // @note regtest's maturity is 1, modify in init.cpp
...
const int32_t BRANCH_CHAIN_CREATE_COIN_MATURITY = 1054080; // 半年才能赎回, 1054080块 * 15s/块 = 183天 . 设定比较长的时间主要防止恶意创建很多支链。
int32_t BRANCH_CHAIN_MATURITY = 4000;// 至少需要 4000 块 * 15s/块 = 1000 分钟 = 16.67 hours
const MCAmount MIN_MINE_BRANCH_MORTGAGE = 1000 * COIN; // 抵押挖矿最小值
int32_t REDEEM_SAFE_HEIGHT = 10800; // 10800 * 8s = 1 day (branch chain block time) 挖矿币安全高度（继续挖矿和赎回需要满足的高度）、举报高度
int32_t REPORT_OUTOF_HEIGHT = 5760; // 5760 * 15s = 1 day
int32_t REPORT_LOCK_COIN_HEIGHT = 60; // 60 * 15s = 15 mins
