...
#define SELECTPARAMS(a) {ECC_Stop();SelectParams(a);ECC_Start();}
...
{
    UniValue tests = read_json(std::string(json_tests::base58_keys_valid));
...
{
    UniValue tests = read_json(std::string(json_tests::base58_keys_invalid)); // Negative testcases
    MagnaChainSecret secret;
    MagnaChainAddress addr;
