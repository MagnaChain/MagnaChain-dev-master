...
BOOST_AUTO_TEST_CASE(tx_invalid)
{
    ...
    for (unsigned int idx = 0; idx < tests.size(); idx++) {
        ...
        if (test[0].isArray())
        {
            ...
            MCValidationState state;
            fValid = CheckTransaction(tx, state, true, nullptr, nullptr, false, nullptr, pcoinsTip) && state.IsValid();
            if (tx.IsCoinBase())
            {
...
BOOST_AUTO_TEST_CASE(basic_transaction_tests)
{
    ...
    stream >> tx;
    MCValidationState state;
    BOOST_CHECK_MESSAGE(CheckTransaction(tx, state, true, nullptr, nullptr, false, nullptr, pcoinsTip) && state.IsValid(), "Simple deserialized transaction should be valid.");

    // Check that duplicate txins fail
    tx.vin.push_back(tx.vin[0]);
    BOOST_CHECK_MESSAGE(!CheckTransaction(tx, state, true, nullptr, nullptr, false, nullptr, pcoinsTip) || !state.IsValid(), "Transaction with duplicate txins should be invalid.");
}
