...
inline int set_error(magnachainconsensus_error* ret, magnachainconsensus_error serror)
{
    if (ret)
        *ret = serror;
    return 0;
                                    ...
                                    unsigned int nIn, unsigned int flags, magnachainconsensus_error* err)
{
    if (!verify_flags(flags)) {
        return magnachainconsensus_ERR_INVALID_FLAGS;
    }
    try {
        ...
        if (GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION) != txToLen)
            return set_error(err, magnachainconsensus_ERR_TX_SIZE_MISMATCH);
...
int magnachainconsensus_verify_script_with_amount(const unsigned char *scriptPubKey, unsigned int scriptPubKeyLen, int64_t amount,
                                    const unsigned char *txTo        , unsigned int txToLen,
                                    unsigned int nIn, unsigned int flags, magnachainconsensus_error* err)
{
    MCAmount am(amount);
...
int magnachainconsensus_verify_script(const unsigned char *scriptPubKey, unsigned int scriptPubKeyLen,
                                   const unsigned char *txTo        , unsigned int txToLen,
                                   unsigned int nIn, unsigned int flags, magnachainconsensus_error* err)
{
...
unsigned int magnachainconsensus_version()
{
    // Just use the API version for now
    return magnachainconsensus_API_VER;
}
