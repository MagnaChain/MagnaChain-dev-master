...
bool StringToSeedSpec6(const std::string str, SeedSpec6& ssp6) 
{
	std::vector<std::string> vstrHexNums;
	boost::split(vstrHexNums, str, boost::is_any_of(":"));
	if (vstrHexNums.size() != 17)
	{
		return false;
	}
	...
	for (int i = 0; i < 16; i++)
	{
		const std::string strV = vstrHexNums[i];
		std::vector<unsigned char> vHexNum = ParseHex(strV);
		if (vHexNum.size() != 1)
			return false;

		ssp6.addr[i] = vHexNum[0];
	}

	int port = atoi(vstrHexNums[16]);
	ssp6.port = port;
	return true;
}

std::map<std::string, Sampler> samplers;
void BeginSampler(std::string name)
{
    Sampler& s = samplers[name];
    s.lastTime = GetTimeMillis();
}
...
void EndSampler(std::string name)
{
    ...
    if (it != samplers.end()) {
        int64_t temp = GetTimeMillis();
        Sampler& s = it->second;
        s.totalTime += temp - s.lastTime;
        s.lastTime = temp;
    }
}
