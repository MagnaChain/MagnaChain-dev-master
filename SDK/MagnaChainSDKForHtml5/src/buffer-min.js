'use strict'
var base64={};base64.byteLength=byteLength
...
lookup[(tmp<<4)&0x3F]+'==')}else if(extraBytes===2)
{tmp=(uint8[len-2]<<8)+uint8[len-1]
parts.push(lookup[tmp>>10]+
lookup[(tmp>>4)&0x3F]+
lookup[(tmp<<2)&0x3F]+'=')}
return parts.join('')}
var ieee754={};ieee754.read=function(buffer,offset,isLE,mLen,nBytes)
{var e,m
...
var INSPECT_MAX_BYTES=50;var K_MAX_LENGTH=0x7fffffff
var kMaxLength=K_MAX_LENGTH;Buffer.TYPED_ARRAY_SUPPORT=typedArraySupport()
...
{throw new TypeError('"size" argument must be of type number')}else if(size<0)
...
return buf}
Buffer._fromString=fromString;function fromArrayLike(array)
...
{buf=new Uint8Array(array)}else if(length===undefined)
{buf=new Uint8Array(array,byteOffset)}else
{buf=new Uint8Array(array,byteOffset,length)}
buf.__proto__=Buffer.prototype
return buf}
Buffer._fromArrayBuffer=fromArrayBuffer;function fromObject(obj)
...
{encoding=byteOffset
byteOffset=0}else if(byteOffset>0x7fffffff)
{byteOffset=0x7fffffff}else if(byteOffset<-0x80000000)
...
{if(dir)return-1
else byteOffset=buffer.length-1}else if(byteOffset<0)
{if(dir)byteOffset=0
else return-1}
...
{if(val.length===0)
{return-1}
return arrayIndexOf(buffer,val,byteOffset,encoding,dir)}else if(typeof val==='number')
{val=val&0xFF
...
{encoding='utf8'
length=this.length
offset=0}else if(length===undefined&&typeof offset==='string')
{encoding=offset
length=this.length
offset=0}else if(isFinite(offset))
{offset=offset>>>0
if(isFinite(length))
{length=length>>>0
...
if(codePoint===null)
{codePoint=0xFFFD
bytesPerSequence=1}else if(codePoint>0xFFFF)
{codePoint-=0x10000
...
{var res='';var len=codePoints.length
if(len<=MAX_ARGUMENTS_LENGTH)
{res=String.fromCharCode.apply(String,codePoints)
return res;}
var i=0
while(i<len)
...
var out=''
var cN;for(var i=start;i<end;++i)
{cN=toHexEx(buf[i]);out+=cN;}
return out}
...
if(start<0)
{start+=len
if(start<0)start=0}else if(start>len)
{start=len}
if(end<0)
{end+=len
if(end<0)end=0}else if(end>len)
{end=len}
...
{encoding=start
start=0
end=this.length}else if(typeof end==='string')
{encoding=end
end=this.length}
...
{val=code}}}else if(typeof val==='number')
{val=val&255}
...
leadSurrogate=codePoint
continue}
codePoint=(leadSurrogate-0xD800<<10|codePoint-0xDC00)+0x10000}else if(leadSurrogate)
{if((units-=3)>-1)bytes.push(0xEF,0xBF,0xBD)}
leadSurrogate=null
if(codePoint<0x80)
{if((units-=1)<0)break
bytes.push(codePoint)}else if(codePoint<0x800)
{if((units-=2)<0)break
bytes.push(codePoint>>0x6|0xC0,codePoint&0x3F|0x80)}else if(codePoint<0x10000)
{if((units-=3)<0)break
bytes.push(codePoint>>0xC|0xE0,codePoint>>0x6&0x3F|0x80,codePoint&0x3F|0x80)}else if(codePoint<0x110000)
{if((units-=4)<0)break
bytes.push(codePoint>>0x12|0xF0,codePoint>>0xC&0x3F|0x80,codePoint>>0x6&0x3F|0x80,codePoint&0x3F|0x80)}else
